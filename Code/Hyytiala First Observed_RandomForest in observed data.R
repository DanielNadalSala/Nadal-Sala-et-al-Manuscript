...
Data<-load_object("C:/Users/sala-d/Desktop/Yatir Forest Paper/Data/Hyytiala_Observed.rda")
...
a<-unique(c(which(is.na(Data$GPP)),which(is.na(Data$D)),which(is.na(Data$PAR)),which(is.na(Data$Tmean)),which(is.na(Data$FSWC30))))

ifelse(length(a)>0,DataFill<-DataFill[-a,],DataFill<-DataFill)

DataFill<-data.frame(Tmean=DataFill$Tmean,PAR=DataFill$PAR,FSWC30=DataFill$FSWC30,GPP=DataFill$GPP,D=DataFill$D)
...
ObservedGPP<-Data$GPP
ModeledGPP<-predict(Model,Data)
...
ggplot(data=DataFrame,aes(x=ID,y=Obs))+
  ...
  annotate("text", label="2013",x=250, y=80, 
           hjust=1, size=7, vjust=1,family="sans")+
  annotate("text", label="2014",x=250+365, y=80, 
           hjust=1, size=7, vjust=1,family="sans")+
  annotate("text", label="2015",x=250+365+365, y=80, 
           hjust=1, size=7, vjust=1,family="sans")+
  annotate("text", label="Observed",x=360+365+365, y=60, 
           hjust=1, size=7, vjust=1,family="sans")+
  annotate("text", label="Modeled",x=360+365+365, y=56, 
           hjust=1, size=7, vjust=1,family="sans")
