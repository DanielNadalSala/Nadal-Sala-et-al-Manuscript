...
Observed<-load_object("C:/Users/sala-d/Desktop/Yatir Forest Paper/Data/Yakir_Observed_Limitations.rda")
Modeled<-load_object("C:/Users/sala-d/Desktop/Yatir Forest Paper/Data/Yakir_Modeled_Limitations.rda")
...
###################Here we will calculate the MLED for each day before and after the optimization.

BeforeDF<-data.frame(Tair=Observed$TLimitFraction,
                     D = Observed$DLimitFraction,
                     PAR = Observed$PARLimitFraction,
                     SWC = Observed$SWCLimitFraction)

AfterDF<-data.frame(Tair=ObservedLS$TLimitObs,
                    D = ObservedLS$DLimitObs,
                    PAR = ObservedLS$PARLimitObs,
                    SWC = ObservedLS$SWCLimitObs)
Before<-NULL
After<-NULL

for (i in 1:length(BeforeDF[,1])){
  
  if(is.na(max(BeforeDF[i,])))
  {Before[i]<-NA
  }else{
  Before[i]<-names(BeforeDF)[which(BeforeDF[i,]==max(BeforeDF[i,]))]
  }
  
  if(is.na(max(AfterDF[i,])))
  {After[i]<-NA
  }else{
    After[i]<-names(AfterDF)[which(AfterDF[i,]==max(AfterDF[i,]))]
  }
  
}

#Metric used: Sum of the diagonal divided by the sum of the table
...
BeforeDF<-data.frame(Tair=Modeled$TLimitFraction,
                     D = Modeled$DLimitFraction,
                     PAR = Modeled$PARLimitFraction,
                     SWC = Modeled$SWCLimitFraction)

AfterDF<-data.frame(Tair=ModeledLS$TLimitObs,
                    D = ModeledLS$DLimitObs,
                    PAR = ModeledLS$PARLimitObs,
                    SWC = ModeledLS$SWCLimitObs)
Before<-NULL
After<-NULL
...
#save(Data,file="C:/Users/sala-d/Desktop/Yatir Forest Paper/Data/Yakir_CombinedLimitations.rda")
